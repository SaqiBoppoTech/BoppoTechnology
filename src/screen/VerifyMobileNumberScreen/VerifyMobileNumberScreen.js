...
const VerifyMobileNumberScreen = () => {
  const { goBackToLogin,checkLoginWithEmailOrMobileNumber} = VerifyMobileNumberHooks()
  return (
    <View style={styles.mainContainer}>
      <FocusAwareStatusBar barColor={Colors.WHITE} />
      <ScrollView>
        <View style={styles.imageContainer}>
          <View style={checkLoginWithEmailOrMobileNumber?.condition ? styles.imageSubContainer: styles.loginImageEmailStyle}>
            <Image
              resizeMode={'contain'}
              source={checkLoginWithEmailOrMobileNumber?.condition ? GlobalImage.otpMobileNumberImage : GlobalImage.otpEmailImage}
              style={styles.loginImageStyle}
            />
          </View>
        </View>
        <View style={styles.titleContainer}>
          <TouchableOpacity onPress={() => goBackToLogin(0)} style={styles.arrowContainer}>
            <LeftArrowSvg heigth={CHANGE_BY_MOBILE_DPI(20)} width={CHANGE_BY_MOBILE_DPI(20)} />
          </TouchableOpacity>
