...
const CreateAccountHooks = () => {
  ...
  const renderCreateAccount = ({item, index}) => {
    const selected =
      selectOption == index ? {} : {borderWidth: 1, borderColor: Colors.BLACK};
    const marginRightStyle = index == 0 && {
    ...
    return (
      <TouchableOpacity onPress={selectIndex}>
        <Square
          ...
          textColor={selectOption == index ? Colors.WHITE : Colors.BLACK}
          color={selectOption == index ? Colors.GRAY_DARK : Colors.WHITE}
        />
      </TouchableOpacity>
    );
  };
    ...
    try {
      ...
      if (
        ...
        const response = await axios.post(
          `${BASE_URL}/auth/api/v1/customer/register`,
          regestarationData,
