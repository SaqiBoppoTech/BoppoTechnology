...
import {
  emailAndMobileNumberValidation,
  loginValidation,
...
const ForgetPasswordHooks = () => {
  ...
  let checkThisIsMobileNumber =
    regex.analyisStartingFirst.test(emailAndPhoneNumber);
  ...
  const navigationToForgotPasswordOtp = async () => {
    try {
      ...
      let checkRegex = checkThisIsMobileNumber
        ? regex.mobileNumber.test(emailAndPhoneNumber)
        : regex.email.test(emailAndPhoneNumber);
      if (checkRegex) {
        const response = await axios.post(
          `${BASE_URL}/auth/api/v1/customer/forget-password`,
          forgotPasswordData,
          ...
          dispatch(
            UserAction.setToastedAlert({
              ...
              toastedAlertText: `OTP has been send to\n${
                checkThisIsMobileNumber ? '+' : ''
              }${emailAndPhoneNumber}`,
            }),
          );
          navigation.navigate(ScreenNames.FORGET_PASSWORD_OTP_SCREEN, {
            sendTo: emailAndPhoneNumber,
          });
        } else {
          dispatch(UserAction.setGlobalLoader(false));
  ...
  return {
    navigationToForgotPasswordOtp,
    emailAndPhoneNumber,
    setEmailAndPhoneNumber,
    checkThisIsMobileNumber,
    checkLengthOfInput,
  };
};
